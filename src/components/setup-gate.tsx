...
function steps(state: SetupState): Step[] {
  return [
    ...
    {
      label: "Item codes",
      detail: "The components you send out and the coated codes that come back.",
      href: "/masters/items",
      done: state.hasItems,
    },
    {
      label: "Job workers",
      detail: "Each one gets its own stock location automatically.",
      href: "/masters/parties",
      done: state.hasJobWorkers,
    },
    {
      label: "Process routes",
      detail: "Which coated code comes back for each component, and at what rate.",
      href: "/masters/routes",
      done: state.hasItems && state.hasJobWorkers,
    },
    {
      label: "Opening stock",
      detail: "A physical count, so balances start from the truth.",
